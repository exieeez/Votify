...
#   main    — сейчас это ветка ПК (ветка по умолчанию), папки android/ в ней нет;
#   android — ветка телефона (лежит ПК-код + папка android/), но код там старше;
#   arena/* — временные ветки сессий Arena, из них самая свежая 01a0ae5a-votify
#             (в ней и новые иконки, и правки веб-версии).
#
...
План:
  1. создать ветку $PC_BRANCH      — версия для ПК (свежий код + файлы design/ из main)
  ...
  Обе новые ветки содержат весь проект целиком: это один кодекс с двумя линиями релизов
  (APK для телефона и сборка для ПК), так что расхождений по файлам между ними нет.
...
# В main лежат дизайн-исходники (zips/jpg), которых нет в ветке телефона, — возвращаем их,
# чтобы переезд на новую ветку ничего не потерял.
if git rev-parse --verify --quiet origin/main >/dev/null; then
  RESTORE=$(git diff --name-only --diff-filter=D origin/main "$PC_BRANCH" -- . ':(exclude)android' || true)
  if [ -n "$RESTORE" ]; then
    say "  возвращаю файлы, которые были только в main:"
    echo "$RESTORE" | sed 's/^/    /'
    if [ "$DRY_RUN" = "0" ]; then
      # shellcheck disable=SC2086
      git checkout origin/main -- $RESTORE
      git commit -q -m "pc: вернуть дизайн-исходники из main" || true
    fi
  fi
fi
