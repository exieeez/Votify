...
## Иконка приложения (Настройки → Иконка приложения)

Первый раздел настроек: варианты иконки на рабочем столе. Переключение делает система —
у каждого варианта свой `activity-alias` на `MainActivity` (сама Activity intent-фильтра
не имеет, иначе на рабочем столе висели бы две иконки):

| Вариант | Алиас | Что это |
|---|---|---|
| Как сейчас | `.VotifyLauncher.Current` | текущая иконка Votify |
| Ч/Б | `.VotifyLauncher.Bw` | та же иконка без серой полосы |
| В духе iOS 27 | `.VotifyLauncher.Glass` | старый логотип: стеклянная плитка, блик, светлая кромка |
| Старая | `.VotifyLauncher.Classic` | старый логотип как есть: тёмная плитка, светлый диск |
| Своя иконка | `.VotifyLauncher.Custom` | картинка из галереи |

Выбор хранится в `CustomPrefs.appIcon` (значит, уезжает в облако/пресеты), а при запуске
`MainActivity` сверяет сохранённый вариант с текущим и при необходимости переключает —
на новом устройстве иконка подтягивается автоматически.

**Своя картинка.** Android не разрешает приложению подменить иконку в лаунчере произвольной
картинкой (иконка — часть APK, см. `android:icon`). Поэтому:

* Android 16+ (One UI 8, Pixel Launcher) — строка «Своя картинка в системе» открывает
  системный диалог смены иконки (`CATEGORY_LAUNCHER_APP_ICON_OVERRIDE`), где систему
  иконку подменяет сама;
* любой Android — строка «Ярлык с картинкой» кладёт на рабочий стол ярлык через
  `ShortcutManager.requestPinShortcut` с картинкой пользователя;
* внутри приложения картинка видна сразу (превью в настройках).

Картинка сохраняется как квадратная PNG 288×288 со скруглёнными углами:
`filesDir/launcher_icon.png`.

Ресурсы иконок генерируются скриптом `scripts/generate-app-icons.sh` (ImageMagick, из
корня репозитория) — он же обновляет превью `assets/app-icon-variants.png`.
