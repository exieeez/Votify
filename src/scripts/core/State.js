...
export const actions = {
  ...
  setSidebarCollapsed(c) {
    store.sidebarCollapsed = c;
  },
  setSidebarMobileOpen(o) {
    store.sidebarMobileOpen = o;
  },
  setTheme(t) {
    store.theme = t;
  ...
  setWaveTracks(tracks) {
    store.waveTracks = tracks;
  },
  setWaveSections(sections) {
    store.waveSections = sections;
  },

  // Charts
  setCharts(type, tracks) {
    store.charts = { ...store.charts, [type]: tracks };
  },
  ...
  // Loading
  setLoading(key, v) {
    store.loading = { ...store.loading, [key]: v };
  },

  // Errors
  setError(key, err) {
    store.errors = { ...store.errors, [key]: err };
  },
  clearError(key) {
    const e = { ...store.errors };
    delete e[key];
    store.errors = e;
  },
};
